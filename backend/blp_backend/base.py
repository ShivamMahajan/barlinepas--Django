...
def send_qr_code(uuuid,saletype,customer,merchant):
   qr_code = get_qr_code(uuuid)
   status = False
   if qr_code:
     status = email_qrcode_venue(qr_code,saletype,customer,merchant)
   return status
...
def email_qrcode_venue(qr_code, saletype, customer,merchant):
...
def send_email(to,subject,texttemplate,htmltemplate,replacements):
   try:
     ...
     print "Error on email attempt %s" % ( e.__str__() )
     return False
...
def send_venue_registration_email(venue):
   ...
   subject = get_email_subject("REGISTRATION")
   emailtemplate = render_email_template(text,replacements)
   texttemplate = render_text_template(text,replacements)
   return send_email( venue.blp_merchant_email, subject, texttemplate, emailtemplate, replacements)
...
class User(object):
  ...
  def login( username,  password ):
    ...
    return False
  @staticmethod
  def social_login(email_address):
    ...
    if len(found_results) > 0:
      ...
      newSocialCustomer = Customer(blp_customer_email=email_address, blp_customer_password=hashlib.md5( str(uuid.uuid4()) ),
		blp_customer_username=email_address, blp_customer_active=1)
...
class Venue(object):
  ...
  def register( venue_data ):
     ...
     if len(found_results)==0:
	   try:
	     venue = Merchant(blp_merchant_name=venue_data['venueName'],
			  blp_merchant_password=hashlib.md5( venue_data['venuePassword'] ).hexdigest(),
			  blp_merchant_email=venue_data['venueEmail'],
			  blp_merchant_city=venue_data['venueCity'],
			  blp_merchant_phone_number=venue_data['venuePhoneNumber'],
			  blp_merchant_active=1,
			  blp_merchant_monday_unit_pricing="0.00",
			  blp_merchant_monday_qty_total=0,
			  blp_merchant_monday_qty=0,
			  blp_merchant_tuesday_unit_pricing="0.00",
			  blp_merchant_tuesday_qty_total=0,
			  blp_merchant_tuesday_qty=0,
			  blp_merchant_wednesday_unit_pricing="0.00",
			  blp_merchant_wednesday_qty_total=0,
			  blp_merchant_wednesday_qty=0,
			  blp_merchant_thursday_unit_pricing="0.00",
			  blp_merchant_thursday_qty_total=0,
		 	  blp_merchant_thursday_qty=0,
			blp_merchant_friday_unit_pricing="0.00",
			 blp_merchant_friday_qty=0,
			 blp_merchant_friday_qty_total=0,
			 blp_merchant_saturday_unit_pricing="0.00",
			 blp_merchant_saturday_qty=0,
			 blp_merchant_saturday_qty_total=0,
			blp_merchant_sunday_unit_pricing="0.00",
		  	 blp_merchant_sunday_qty=0,
			 blp_merchant_sunday_qty_total=0
